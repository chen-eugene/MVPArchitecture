 ...
 */
package com.eugene.core.module.imageloader
 ...
 */
@Singleton
class ImageLoader<T : ImageConfig> @Inject constructor() {
    /**
     * 可在运行时随意切换 [IImageLoaderStrategy]
     *
     * @param strategy
     */
    @set:Inject
    var loadImgStrategy: IImageLoaderStrategy<T>? = null
    ...
    fun loadImage(context: Context, config: T) {
        this.loadImgStrategy?.loadImage(context, config)
    }
    ...
    fun clear(context: Context, config: T) {
        this.loadImgStrategy?.clear(context, config)
    }
}
